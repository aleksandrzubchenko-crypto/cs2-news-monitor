...
def extract_one(psd_path, out_root, slug=None):
    ...
    # reference (оригинал как есть)
    psd.composite(force=True, viewport=(0, 0, W, H)).convert("RGB") \
        .save(os.path.join(out_dir, "reference.png"))

    photo = pick_photo(leaves, W, H)
    ...
    team_logos = [l for l in leaves if is_raster(l) and l is not photo
                  and any(k in (l.name or "").lower() for k in TEAM_KW)
                  and not any(k in (l.name or "").lower() for k in BRAND_KW)]
    # прячем из рамки ТОЛЬКО динамику; статичный бренд-текст остаётся запечён
    dynamic = set([photo] if photo else []) | set(dyn_txt) | set(team_logos)

    # FRAME. Фото прямоугольное, поэтому:
    #  1) находим ГРАНИЦЫ видимого фото через diff «с фото / без фото» (clip-корректно);
    #  2) пробиваем в comp_full СПЛОШНУЮ прямоугольную дыру по этому bbox (чисто, без
    #     полупрозрачных просветов синего light-фона — та самая фиолетовая грязь);
    #  3) возвращаем над-фото элементы (полоса/кавычка/линия/вотермарки/лого) оверлеем.
    # Панель НИЖЕ фото не трогаем (она вне photo_bbox).
    ...
    pi = leaves.index(photo) if photo else 0
    ...
    _ID.Draw(alpha).rectangle([pb[0], pb[1], pb[2]-1, pb[3]-1], fill=0)   # сплошная дыра
    ...
    for l in sorted(dyn_txt, key=lambda t: bbox_of(t)[1]):     # сверху вниз
        ...
        zt.append({"name": l.name, "bbox": b, "font_px": fp, "nlines": nlines,
                   "color": list(rgb) if rgb else [255, 255, 255], "align": align,
                   "sample": sample[:80]})
    tl_slot = bbox_of(team_logos[0]) if team_logos else None
    zones = {"canvas": [W, H], "photo_bbox": list(photo_bbox),
             "texts": zt, "team_logo_slot": tl_slot, "slug": slug,
             "source_psd": os.path.basename(psd_path)}
